...
#            input_sv['prediction']=int(pred_sv)
    ...
    #5. generate summary plot for input and benchmark sv data     
      output$scatter1 <-  renderPlotly({
            ...
            p2<- all_benchmark_sv %>% plot_ly(type = 'scatter3d',mode = 'markers',x=~-log(prediction_TA,10), y=~-log(prediction_TG,10),z=~-log(prediction_TS,10),size=~-log(predict_max,10)*10, color=~factor(label),  colors = c("grey","green", "purple"), hovertemplate = paste("sv_type:",all_benchmark_sv$sv_type,"<br>sv_chr:",all_benchmark_sv$sv_chr,"<br>sv_chr2:",all_benchmark_sv$sv_chr2,"<br>sv_bp_st:",all_benchmark_sv$sv_bp_st,"<br>sv_bp_end:",all_benchmark_sv$sv_bp_end, "<br>TA_SV_prob:", round(all_benchmark_sv$prediction_TA,2),"<br>TG_SV_prob:", round(all_benchmark_sv$prediction_TG,2),"<br>TS_SV_prob:", round(all_benchmark_sv$prediction_TS,2),"<br>sv_database:",all_benchmark_sv$sv_database),opacity=0.2) 
